...
| B42 | Wrayth scripts/strings "Belongs in Lich" counts untested | — | `scriptsCount` and `stringsCount` are populated by the Wrayth parser but could not be verified against a real `settings.xml` with populated `<scripts>` or `<strings>` blocks. Needs a test file with those sections to confirm the count and confirm the "Belongs in Lich" section renders in the import wizard Step 3. |
