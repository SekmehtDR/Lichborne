...
All user state is round-tripped to YAML in `profiles/` inside Electron's userData (`%APPDATA%\lichborne\profiles\` in production — lowercase because Electron's `app.getName()` reads the top-level `name` field, not `build.productName`). **Do NOT** put profiles inside the install directory — pre-v0.6.4 did, and every NSIS upgrade wiped them because the previous version's uninstaller runs before the new build is extracted. `ensureProfilesDir()` in [profiles.ts](src/main/profiles.ts) is the canonical accessor (creates dir if missing); a two-stage migration (NSIS `preInit` hook in [build/installer.nsh](build/installer.nsh) + runtime fallback) copies any pre-v0.6.4 install-dir profiles into userData. **Casing pitfalls:** when writing NSIS or any path-mention in docs, use lowercase `lichborne` — `Lichborne` is only the installer/menu display name.
