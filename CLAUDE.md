...
Main text rendering uses **react-virtuoso**. Scroll pinning is a delicate state machine: `pinnedRef` snapshots before commit, `suppressUntilRef` gates programmatic scrolls, `onWheel` un-pins synchronously. Smooth vs. instant scroll is decided by **`smoothActive()`** = `settings.smoothScroll && !floodRef.current` — NOT the raw setting. The `floodRef` half is flood-adaptive (B82): a burst of incoming lines (login dump, heavy combat) trips `floodRef`, forcing instant scroll so smooth scroll's re-virtualization sweep can't saturate the render pipeline. The trip threshold is the player-tunable `settings.smoothScrollBurstLimit` (B83, default 25), read in the game-event handler via a `floodThresholdRef` mirror. All five smooth/instant decision points route through `smoothActive()`. **Don't add `useEffect`-driven scroll logic** without reading GameWindow.tsx and DESIGN.md §23. Room-state updates funnel through the rAF pump (`roomQueueRef`), not direct `setRoomState` — see DESIGN.md §23 and pitfall #20.
...
**Two opt-out settings (v0.6.9).** `smoothScroll` (default OFF) gates story-window smooth scroll AND the map camera glide. `mapAnimations` (default ON) gates the per-color category effects. Both are per-character `AppSettings` fields, persist via the dynamic profile `state:` pipeline, and thread `GameWindow → PanelFrame → MapPanel → GenieMapView` (the `autoLinkUrls` path).
...
23. **For any new story-window scroll decision, gate on `smoothActive()`, not `settings.smoothScroll`.** `smoothActive()` = `settings.smoothScroll && !floodRef.current` — the flood half forces instant scroll during a line burst so smooth scroll's re-virtualization sweep can't saturate the render pipeline (B82). Using the raw setting reintroduces the 4K startup-lag bug.
