...
28. **Panel Manager has two ownership dimensions; don't conflate "slot added" with "slot has streams".** v0.8.1 introduced explicit add/remove of panel locations — the 4 slots (Main-Top, Top-Right, Middle-Right, Bottom-Right) each carry an `*Added` boolean (`mainTopAdded` / `topAdded` / `midAdded` / `bottomAdded`) that controls whether the slot renders in the game window. The slot's `tabs` array controls which streams live in it. The two are independent: an added slot with empty tabs renders an `EmptyPanelSlot` placeholder (it's still in the layout, ready to fill); a removed slot with leftover tabs would be a bug — `removePanelZone` clears the tabs at the same time it flips the flag. New code that mutates tabs (`addToZone`, `moveTabToZone`) auto-flips the `*Added` flag on as a safety so adding a stream to a removed slot doesn't leave it invisible. Migration defaults when the flag is missing: **Main-Top → false** (new zone, explicit opt-in), other three → **true** (preserve v0.8.0 always-visible). When adding a fifth panel location in the future, follow the same pattern: new `*Added` state + scopedKey persistence + migration default + render-gate in GameWindow + `addPanelZone`/`removePanelZone` handlers + PanelManager UI entry.
