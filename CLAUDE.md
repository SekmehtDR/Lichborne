...
The className picks one or the other (`!mapAnimations ? 'genie-anim-off' : (isDragging||inMotion) ? 'genie-pan-dragging' : ''`) so the ping-pause and ping-exemption rules never collide. Why the pause at all: profiling showed Layerize + Recalculate Style + Layout consuming 50%+ of frame budget on dense zones during drag/walk.
