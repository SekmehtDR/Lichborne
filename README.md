...
- **Hand-bar tracking depends on what the game sends, too.** DragonRealms doesn't send a hand update for *every* action that puts an item in your hand (event/quest items with custom verbs are the usual gap), so the `L`/`R` hand slots can occasionally show `Empty` while you're holding something. As of v0.13.2 a `GLANCE` always resyncs them — Lichborne reads the glance text itself, the same trick the Profanity front-end uses.
