...
- **Two map views, both Lich-native** — a **Lich Map** view that displays Lich's built-in map artwork with your current room highlighted, and a **Lich Graph** view that auto-lays out every room in your local neighborhood from Lich's own walk data — no Genie required. Point it at your Genie maps folder (optional) and rooms light up with district tints, landmark icons (shops, healers, stat trainers, etc.), richer tooltips, and dashed fallback edges for connections Lich doesn't know about yet. Map follows you as you walk; click any room to pathfind to it. A togglable legend (▤) shows what every color, shape, and glyph means — and lets you turn individual visual layers on or off if you prefer a cleaner view. Search by room name or by Lich room ID.
