...
- **Lay it out your way — Static or Windowed Panels.** By default your panels sit docked in tidy zones (**Static Panels**). Flip to **Windowed Panels** (in the Panels manager) and your whole layout floats — the main game text, the input bar, the vitals bar, the status icons, and every panel become independent windows you can drag, resize, and **snap together** (they click flush to each other and the screen edges, with guide lines to help you tile). Add as many panel windows as you want, hide a window's title bar for a compact look, and **lock** the layout once it's just right so nothing moves by accident. **Drag a stream tab along its tab bar to reorder it** — the other tabs slide out of the way so you can see exactly where it'll land (works in both layouts; locked windows keep their tab order fixed too). It's per-character, saved across launches, and travels with Transfer The **Layout** button on the top bar opens the **Layout Manager**, where both modes are described side by side and you can switch between them.
- **Lichborne Experiences — see your room as a living scene.** The **Experiences** button opens a shelf of graphical surfaces that float over your layout (your panels and streams are untouched). The first is the **Living Tableau [Beta]**: everyone in your room becomes an avatar in their Contact colors, speech blooms as comic bubbles (whispers dotted and private, emotes as action captions), telepathic **thoughts collect in a quiet log in the bottom-left corner** (newest at the bottom, older ones fading up and away) so they never cover the scene, arrivals slide in from the direction they came and departures walk out as fading ghosts, hiders and the invisible show as shadows, and the dead lie greyed until a resurrection stands them up. The seating itself shows the conversation — talkers gather toward the middle (you included), pairs in conversation drift together, big gatherings spread into an amphitheater. Creatures stand as their own figures in the game's monsterbold color (four blademasters are four monsters); your Contacts wear a ✦ and their figure is clickable for their contact card; and your own avatar shows your condition (hidden, dead, bleeding, and the rest). **And when you fight, the scene turns into a combat cockpit:** three readiness rings wrap your avatar (roundtime, cast, aim, each in its own color), your avatar pulses when you're stunned or bleeding, and a small panel beneath you reads your **balance** and **position** as red→yellow→green gauges. Run **`ASSESS`** and the creatures arrange by where they actually stand — *facing you*, *flanking*, or *behind you* — with your current target ringed in gold, the ones in melee glowing as they press the attack, and any that are reeling shown off-balance; **click a creature to turn and face it.** The ones you fell stay as marked corpses until they decay, and the balance/position gauges ease off once the fight is over. (Combat is Beta — it costs nothing until a fight, and reads from the same combat text you already see.) Hover the scene's window for its own controls: **A− / A+** size all the scene's text to taste, and **⚙ "Show in this scene"** lets you tick exactly what you want — speech bubbles, yells, whispers, thoughts, emotes, creatures, arrivals & departures, plus the combat layers (readiness rings, threat markers, danger pulse, combat gauges) — each remembered per window. It costs nothing until you open it, your theme and epilepsy-safe settings apply, and the game text always remains the source of truth. **The second Experience is Moons [Beta]**: Elanthia's three moons — soot-black Katamba under its faint miasmatic haze, blood-red Yavash glowing through its ruby cloud deck, silvery-blue Xibar crisp and airless — arc across a living sky positioned by how long each has left, then travel a shallow underground path back to where they'll rise, so nothing ever just disappears. A golden **sun** rides the same arc, placed from the same community data the moonwatch script shares plus the game's own sunrise and sunset announcements. The whole sky lives: bright at midday, warm glows at sunrise and sunset, deepening night with stars that fade in through twilight, a mountain silhouette along the horizon — and anything about to rise or set breathes gentle horizon rings (all motion respects your epilepsy-safe setting). The daylight **follows the sun** — a warm glow blooms around it, brightening at noon and turning golden as it sinks to sunrise/sunset, lingering as an afterglow through dusk and returning before dawn — and each **moon is lit from the sun's direction**, so you see it wax and wane in its own colours (ruby Yavash, silvery-blue Xibar, sooty Katamba). The sun and moons **sink behind the horizon** and disappear while set, then crest back up, and the **landscape falls into shadow at night and lights up by day** — the shade even sweeps in from the side opposite where the sun is rising or setting. Below the horizon a little **wilderness** has grown in — a distant forest, foreground trees, a winding **stream** and a **lake that mirrors the sun and moons** on its surface — with real depth (nearer things larger) that lightens by day and darkens by night; the **trees cast shadows that follow the sun**, swinging from straight back at midday to long diagonals near sunset, and the whole scene **dresses itself by season** — snow-capped trees and an iced-over lake in winter, blossoms in spring, lush green in summer, and autumn colours with **falling leaves** in fall (toggle it under the scene's ⚙ "Trees & water"). **Sunrise and sunset rays** fan across the ground, and a frosted **at-a-glance panel** above the footer keeps every body's next rise/set time in view the whole while. After dark the **stars twinkle** — and the sky fills in as the night deepens, brighter stars first and fainter ones **emerging the closer it gets to midnight**, with **shooting stars** streaking from all across the sky on clear nights; clear **winter** nights shimmer with **aurora**, and **summer** evenings drift with **fireflies**, while winter dusts the ground and ridgeline with **snow**. Live "sets in 88m" / "rises in 152m" countdowns throughout, **hover any body for its lore** (Katamba burnt by the World Dragon's breath…) plus the clock time it next rises or sets, and the footer tracks the next event and how fresh the data is. The window's **⚙** menu toggles each layer independently — sun, living sky, countdowns, names, silhouette, transition effects, weather, weather effects, date — remembered per window like the Tableau's. And if floating windows aren't your style, **any Experience can live as a regular panel tab instead**: every panel's **+** button lists the Experiences below a separator, marked with an **[e]** badge — tuck the sky in beside your Thoughts tab and save the screen space. It's powered by the community **moonwatch** script (run `;moonwatch window` on any Lich character). A **header strip** across the top reads the sky at a glance — day or night with the sun's countdown, the next moon, and the current **weather** — while a bold word on the sky itself names the **time of day** (DAY, NIGHT, DAWN, DUSK) — with the finer Elanthian daypart in parens once you've checked, like *Night (late evening)* — and a **footer strip** carries the **Elanthian date** (e.g. *4 Ka'len the Sea Drake · 457 A.V. · ❄️ winter*, with a season icon). A single **⟳** refreshes the weather and date — and it does so **silently**, sending the game's `WEATHER` and `TIME` commands behind the scenes so nothing clutters your game window (each is its own **⚙** toggle); the weather also updates on its own whenever you glance at the sky. And the sky **reacts to the weather it reports** — clouds drift in, snow and rain fall (slanting with the wind), fog settles on the horizon, a storm flickers with lightning (all respecting your epilepsy-safe setting, toggleable on their own ⚙ layer). Both strips and all the sky text adapt to day and night so they stay readable against any sky
...
- **Make important text pop — text effects** — a highlight (or a contact's name) can do more than recolor: pick **Glow, Shimmer, Rainbow, Pulse, Gold, Gradient, Fire, or Frost** and the matching text animates so it catches your eye mid-scroll. Set it right in the highlight/contact-template editor. (Any old "glow" you had keeps working, and every effect holds still when epilepsy-safe mode is on.)
...
- **Never miss your free SimuCoins** — Simutronics hands subscribers free SimuCoins every month, but they only land if you remember to claim them. Turn it on for an account in **Settings → SimuCoins** and Lichborne checks the store when you start the client: coins waiting light the **coin** in the top bar up as **polished gold with a shine sweeping across it** and show the count, one click on it collects them from every account at once, and **Auto-claim** does it for you if you'd rather not think about it. Nothing to claim? The coin sits dull and quiet — and if you never enable it, it never appears at all. (Epilepsy-safe mode keeps the gold and drops the shimmer.) Also `/simucoin`, `/simucoin claim`, or just `/sc`. **Nothing is sent until you enable it for an account**, and the Settings section spells out exactly what happens right above the switch that does it: Lichborne signs in to Simutronics' own store over HTTPS with the account password you already saved, reads your balance, and signs out — no store data kept on disk, checked once per launch, never polled in the background. *(Thanks to Thires, whose [Genie SimuCoins plugin](https://github.com/Thires/SimuCoins) showed how this works.)*
...
**macOS (beta, v0.18.0+, Apple Silicon):** Download the `.dmg` and drag Lichborne to Applications. **First launch needs one Terminal command.** Because Lichborne doesn't buy Apple's $99/yr developer certificate, macOS quarantines the download — and on Apple Silicon it words that as **"Lichborne is damaged and can't be opened"**, which is alarming and misleading: the download is fine, it's just not notarized by Apple. Clear the quarantine flag and it launches normally from then on:

```bash
xattr -cr /Applications/Lichborne.app
```

If macOS instead offers **System Settings → Privacy & Security → "Open Anyway"**, that works too — either route is one-time. The app is safe and the source is public. **Mac updates are manual for now** (auto-update requires that certificate) — grab new versions from the Releases page. Mac keyboard conventions (Cmd+F, Cmd+1–9, Cmd+Shift+Enter) work alongside the Ctrl versions.
...
- No code signing yet on any platform — Windows SmartScreen → **More info → Run anyway**; macOS quarantines the download and may call it **"damaged"** (it isn't — that's how Apple Silicon words "not notarized"), cleared once with `xattr -cr /Applications/Lichborne.app`
